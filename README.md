...
export const DagProvider = ({ children }: DagProviderProps) => {
  const [dagData, setDagData] = useState<DagData>({
    dag_name: '',
    description: '',
    active_flag: '',
    max_active_runs: '',
...
const DagDetails: React.FC = () => {
  ...
  const { headers, dagData, updateDagData, setValidateDagDetailsForm } = dagContext;
...
const DagParams: React.FC = () => {
  const { t } = useTranslation();
  const dagContext = useDagContext();
  const { headers, dagData, updateDagData, setValidateDagParamsForm } = dagContext;
  ...
  }, [headersData, dagData, setValidateDagParamsForm]);
  ...
  return (
    <Box className="tw-mt-5 tw-block tw-pb-2 tw-border-solid tw-border-2 tw-border-pfizerBlue">
      ...
      >
        {({ values, handleBlur, setFieldValue, errors, touched }) => (
          <Form id="dag-params-form">
            <Box display="inline-flex" alignItems="center" padding={2} sx={{ '& .MuiFormControl-root': { width: '20vw', maxWidth: '20vw' } }}>
